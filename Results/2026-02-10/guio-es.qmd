...
Para abririlos el nombre es la dirección mientras que si dices solo el nombre programa busca en donde estás.
...
:::
Se ve con  con zless y nombre y se sale Q, gunzip -c del nombre barra vertical wc-l(descomprimo y paso a prgrama workconut) de esta manera evito descomprimir/ (zless -s). WC cuenta y zless ver
...
:::{.callout-note icon=false}
## Preguntas
- ¿Crees que deberíamos guardar los archivos descargados en `../../data`? si pero ocupa demasiado y como tenemos un scrip de descarga automatica de hecho es lo bueno de todo el texto anterior
- ¿Cómo podrías guardar los mensajes de `wget` en un archivo? Orden de arriba elimina, programa de descargas que dan datos 
- ¿De cuántas secuencias diferentes se compone este genoma de referencia?
Grep: que muestre las líneas en donde esté la expresión "^caracteres en comun entre ellas y el signo de mayor " y el archivo ::: hay tres lineas
:::
