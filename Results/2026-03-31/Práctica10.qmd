...
Ejercicio ¿Puedes confirmar el número de secuencias? Sí 42 ¿Qué longitud tiene el alineamiento? El total sale 697326/42=16.603 \>library(ape) \>aln \<- read.dna("../../data/mtDNA.fasta", format = "fasta") \>view(aln: nos muestra las secuencias ordenadas con el nt en cada secuenci)

¿Qué función usarías para extraer los nombres de las secuencias? \>names(mtdna).
...
Ejercicio ¿De qué clase son los objetos mt_nj y mt_upgma? Consulta la estructura de estos objetos (str(mt_nj)). Listas de características del árbol filogenético con las especies en que nodos se encientran y la distancia a los nodos. Estructura: fila de 4 con los nodos y la distancia de estos con la especie corespondiente. List of 4 \$ edge : int \[1:81, 1:2\] 47 47 54 54 53 53 52 52 51 51 ... \$ edge.length: num \[1:81\] 6.03e-05 5.20e-08 -4.37e-08 4.37e-08 8.51e-08 ... \$ tip.label : chr \[1:42\] "G.gorilla_D38114" "P.troglodytes_JF727212" "P.troglodytes_JN191207" "P.troglodytes_JF727168" ... \$ Nnode : int 40 - attr(*, "class")= chr "phylo" - attr(*, "order")= chr "postorder"
...
Para qué sirven las funciones add.scale.bar() y axisPhylo()? This function adds a horizontal bar giving the scale of the branch lengths to a plot of a phylogenetic tree on the current graphical device. Lo que hace es añadir una barra la longitud de cada rama. add.scale.bar(x, y, length = NULL, ask = FALSE, lwd = 1, lcol = "black", ...)

axisPhylo(side = NULL, root.time = NULL, backward = TRUE, ...) This function adds a scaled axis on the side of a phylogeny plot. Eje relacion de tiempo con otro eje (root.time basado en el la raíz). Side es un valor en localizacion cada lugar y después backward es una funcion que da la vuelta al eje de tiempo para poner 0 en raiz.
...
A continuación se escoge la posición de la raíz que optimiza la regresión entre distancia de raíz a puntas y edades de las puntas (Rambaut et al. 2016). Quiere decir, que conociendo el año en que dejan de evolucionar y la distancia que se conoce en cada especie. La regresión utiliza cada raíz posible para obtener lo que se conoce hasta encontrar el más probable.
...
Ejercicio -¿Se corresponden bien las distancias evolutivas estimadas con las dataciones? NO, se ve que Denisovanos parecen terminar de evolucionar después que los chimpancés cuando sabemos que es lo contráreo -¿Qué razones puede haber para que la edad de una secuencia no se corresponda con la cantidad de cambios que ha acumulado? Errores dataciones suponiendo más antiguos de lo que son, y algunos de estos tienen errores. También estas secuencias tieen cambios no mutaciones ancestrales sino, son productos de degradación del Dna. O comprobar en tabla apéndice y puede que haya errorres en tabla.

-¿Qué secuencias crees que se ajustan menos a su supuesta edad? H.s. denisova
...
¿Cuántas veces mayor es la distancia entre humanos y chimpancés que entre humanos y neandertales? 0,09104153/0,01214282=7,49 ¿Cómo calcularías la distancia media entre dos secuencias de humanos modernos? 0.003747961
