...
curl_download('https://www.ebi.ac.uk/ena/portal/api/search?result=assembly&query=tax_tree(6157) AND genome_representation="full"&fields=version,tax_id,scientific_name,last_updated,base_count&limit=0',
              destfile = 'z1.txt')
```
NO tiene URLencode. El navegador traduce espacios entre una condición y la otra y navegador diferencia especificaciones. Pero al trabajar en R hay que darlo más fácil URLencode codifica unos simbolos para que no se confunda e interprete completamente como pararse en un espacio.

Se pueden con Ctrl + enter: puedo ejecutar la línea en la que estaba.
```{r}
curl_download(
  ...
  destfile = 'z1.txt')
```
Este tiene URLencode mientras que el primero no lo tiene.
```{r}
...
limit  <- 'limit=0'
URL3 <- paste0(portal, result, query, fields, limit)
curl_download(URLencode(URL3), destfile = 'z2.txt')
```
En este en lugar de URL pones el paso completo hasta el documento. Nos da una tabla en R como descrito pero no descarga. Pero si tiene URL.
URL
[1] "https://www.ebi.ac.uk/ena/portal/api/search?result=assembly&query=tax_tree(6157) AND genome_representation=\"full\"&fields=version,tax_id,scientific_name,last_updated,base_count&limit=0". 
Si que funciona (se ve poniendo en consola URL): y es más simple ya que siquiero encontarr algo fácil y se tiene fácil que se debe editar en los 5 pasos de una consulta de ENA.
filtros son query: árbol taxonomico y representacion del genoma completa en este caso

Buscar secuencia relacionada con alcohol DH hay que decidir como hacer. Primero se busca graficamente y después pedirle que muestre la búsqueda en un formato parecido.
https://www.ebi.ac.uk/ena/portal/api/results? nos da resultados para ello buscar.
:::
...
fields <- 'fields=tax_id,description,scientific_name,last_updated,base_count, first_public&' 
limit  <- 'limit=0'
URL <- paste0(portal, result, query, fields, limit)
curl_download(URLencode(URL), destfile = 'z3.txt')
```
curl -X POST -H "Content-Type: application/x-www-form-urlencoded" -d 'result=sequence&query=tax_tree(32281)%20AND%20description%3D%22ADH%22%20AND%20mol_type%3D%22genomic%20dna%22&fields=accession%2Cdescription%2Ctax_id%2Cidentified_by%2Cmol_type&format=tsv' "https://www.ebi.ac.uk/ena/portal/api/search"
en la tabla se separa por tabulador
...
si hay apostrofe en el documento cree que es comilla: por ello no usar carracter abrir comilla. "quote=""). Tabla guardad en objeto llamado adh.
para ver filas y columnas: dim(adh) 1] 1225    7
para ver el comienzo primeras lineas: head(adh)
datos:names(adh)
[1] "accession"       "tax_id"          "description"     "scientific_name" "last_updated"
[6] "base_count"      "first_public"   
[
acceder a la columna: adh$base_count
...
"summary (adh$base_count)"
   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
     18     558     841    1243     957   17499 
3. ¿De cuántas especies diferentes son estas secuencias?
unique(adh$scientific_name) y su longitus: length(unique(adh$scientific_name))
Son 312
4. ¿Cuándo se publicaron las más recientes?
max(adh$first_public)
[1] "2023-06-14"

para primeras fechas head(adh$first_public)
[1] "2006-05-25" "2008-08-06" "2008-08-06" "2008-08-06" "2008-08-06" "2006-12-24"

Otra forma es decir ordenar con "sort" de mayor a menor y al resultado muestre los 6 últimos valores "tail" si al final depués del 2º parántesis se añade ", n=1": tail(sort(adh$first_public))
[1] "2023-06-14" "2023-06-14" "2023-06-14" "2023-06-14" "2023-06-14" "2023-06-14"
:::
...
platelmintos <- read.table(URLencode(URL3),
    header = TRUE,
...
```
lo que quiere es la modificación y que se vea como aumenta la longitud de la secuencia según las publicaciones aparecen (ordenadas por fechas)
:::{.callout-note icon=false}
## Ejercicio 3
