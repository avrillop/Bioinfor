...
Tenemos ~/datasets summary , resumen de la base de datos. annoted que me muestre el formato jason lines y dataformat lo transmora a tsv (pero que ademas datos genoma, los campos que interesan los de detras (nombre organismo, fecha publicacion ensamblaje genoma). Se pueden pedir un montón campos diferentes sobre cada genoma. Para saber que campos. dataformat tsv genome --help: da variables o caracteristicas de genoma (no todos esa info). 
...
~/datasets summary genome taxon Trypanosoma --annotated --as-json-lines: eesto nos da para ver la informacion que le das a datasets.
```{bash}
...
## Desafío
1. ¿Qué diferencia hay entre las líneas 7 y 8 de la tabla anterior?GCF y GCA que son el mismo al fin y al cabo.
2. Busca una función de R que te permita identificar las líneas repetidas en un marco de datos. which(duplicated(mi_df)) cuando se definido mi_df como data frame.
:::
...
podemos usar la función `as.Date()`: Es decir ahora para poder hacer gráficas deben ser nº y no letras alfabéticos para la ordenacion
Read.table se indicó la primera vez al hacer la tabla
...
# Archivos JSON y GFF
Habiendo visto las opciones es decir que todo lo de antes nos permite observar informacion y analizar que es mejor, decidimos descargar el genoma y la anotación funcional
...
`dim(gff)`lo que muestra es el nº de filas 42497 y el nº de columnas 9
...
Estos también se observan con `head(gff)` que combienza con una pequeña tabla deinformación entre las cuales está TYPE: 5. 
